...
RUN \
apk -U add go git && \
cd /build && export REF="$(git describe --tags HEAD || git symbolic-ref --quiet --short HEAD)" && echo "Building ${REF}" && \
git clone -q https://github.com/tonistiigi/dnsdock.git /go/src/github.com/tonistiigi/dnsdock && \
cd /go/src/github.com/tonistiigi/dnsdock && git checkout -q "${REF}" && \
